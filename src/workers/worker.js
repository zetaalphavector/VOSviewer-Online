...
import { LINLOG_MODULARITY, NetworkNormalizer, } from "utils/networkanalysis/NetworkNormalizer";
...
import { errorKeys, fileTypeKeys, minNItems, parameterKeys, processTypes, } from "utils/variables";
...
self.addEventListener("message", (event) => {
  ...
  switch (type) {
    ...
    case "start process data": {
      ...
      const { nNodes, edges, edgeWeights, itemIdToIndex } = _prepareNetworkData(
        options.mapData,
        options.networkData,
      );
      ...
      const nItemsLargestComponent = networkNormalizer.unnormalizedNetwork
        .createSubnetwork(networkComponents, 0)
        .getNNodes();
    ...
    case "start handle unconnected items": {
      ...
      if (itemsToShow) {
        ...
        const filteredNetworkData = options.networkData.filter(
          (d) => itemsToShow.includes(options.itemIdToIndex[d[0]])
            && itemsToShow.includes(options.itemIdToIndex[d[1]]),
        );
    ...
    case "continue run layout":
      if (layoutRandomStart < layoutCreator.nRandomStarts) {
        ...
        self.postMessage({
          ...
          data: {
            progressValue:
              (100 * layoutRandomStart) / layoutCreator.nRandomStarts,
          },
        });
      } else {
    ...
    case "start run clustering":
      self.postMessage({
        type: "update loading screen",
        data: {
          processType: processTypes.RUNNING_CLUSTERING,
          progressValue: 0,
        },
      });
      clusterRandomStart = 0;
      clusteringCreator.init(
        networkNormalizer.normalizedNetwork,
        options,
        networkNormalizer.normalizationMethod === LINLOG_MODULARITY,
      );
      self.postMessage({
    ...
    case "continue run clustering":
      if (clusterRandomStart < clusteringCreator.nRandomStarts) {
        ...
        self.postMessage({
          ...
          data: {
            progressValue:
              (100 * clusterRandomStart) / clusteringCreator.nRandomStarts,
          },
        });
      } else {
        ...
        if (clusteringCreator.mergeSmallClusters) {
          ...
          for (let i = 0; i < bestClustering.nNodes; i++) {
            ...
            bestClustering.cluster.push(
              nItemsPerCluster[cluster] >= clusteringCreator.minClusterSize
                ? cluster
                : null,
            );
          }
...
function _parseJsonFile(jsonFileOrUrl, authToken) {
  if (jsonFileOrUrl) {
    ...
    if (jsonFileOrUrl instanceof File) {
      ...
      reader.onerror = () => {
        const fileError = getFileReaderError(
          reader.error,
          fileTypeKeys.VOSVIEWER_JSON_FILE,
        );
        self.postMessage({
      ...
      fetch(jsonFileOrUrl.url, {
        ...(!!authToken && jsonFileOrUrl.url.includes("zeta-alpha.com")
          ...
          : {}),
        credentials: "include",
        method: jsonFileOrUrl.method,
        body: jsonFileOrUrl.body,
      })
        .then((response) => {
          if (!response.ok) {
...
function _parseJson(jsonObjectOrText) {
  ...
  if (data && data.network && (data.network.items || data.network.links)) {
    ...
    if (jsonData.config && _isPlainObject(jsonData.config.parameters)) {
      jsonData.config.parameters = _reduce(
    ...
    const parseResultsMapData = {
      data: mapData,
      meta: { fields: Object.values(_keys(itemPrototype)) },
    };
    const parseResultsNetworkData = {
      data: networkData,
      meta: { fields: Object.values(_keys(linkPrototype)) },
    };
    ...
    fileError = getFileError(
      errorKeys.FILE_EMPTY,
      fileTypeKeys.VOSVIEWER_JSON_FILE,
    );
  } else {
    fileError = getFileError(
      errorKeys.INVALID_JSON_DATA_FORMAT,
      fileTypeKeys.VOSVIEWER_JSON_FILE,
    );
  }
  self.postMessage({
    ...
    parse(mapFileOrUrl, {
      ...
      complete: (parseResults) => {
        if (parseResults.errors.length) {
          const emptyLastLineErrorIndex = _findIndex(
            parseResults.errors,
            (error) => error.row === parseResults.data.length - 1
        ...
        if (!fileError && networkFileOrUrl) {
          ...
          parse(networkFileOrUrl, {
            ...
            dynamicTyping: true,
            error: (error) => {
              fileError = getFileReaderError(
                error,
                fileTypeKeys.VOSVIEWER_NETWORK_FILE,
              );
              self.postMessage({
                type: "end parse vosviewer-map-network file",
                data: { fileError },
...
function _removeUnconnectedItems(clustering, clusterToShow) {
  const itemsToShow = clustering
    ? clustering.getNodesPerCluster()[clusterToShow]
    : undefined;
  return itemsToShow;
}
